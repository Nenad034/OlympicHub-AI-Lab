...
export const FlightSearchForm: React.FC = () => {
    ...
    const openAirlineDropdown = (e: React.MouseEvent) => {
        e.stopPropagation();
        const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
        setAirlinePanelPos({ top: rect.bottom + 8, left: rect.left });
    ...
    const selectedAirlineLabel = AIRLINES.find(a => a.id === airline)?.name || 'Sve kompanije';
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }} className="v6-flight-form-container">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    ...
                    {tripType !== 'multicity' && (
                        <div className="v6-search-controls-row" style={{ alignItems: 'center', flexWrap: 'nowrap', gap: '8px' }}>
                            ...
                            <button onClick={() => { const tmp = origin; setOrigin(destination); setDestination(tmp); }} className="v6-swap-circle"><ArrowLeftRight className="icon-luxury" size={18} /></button>
                            <AirportAutocomplete value={destination.code} onChange={(code, city) => setDestination({ code, city })} placeholder="Odredište" icon={<Search className="icon-luxury" size={18} />} />
                            <div className="v6-ctrl-box" style={{ flex: 1.5 }}>
                                <div className="v6-input-v6-styled" onClick={() => setShowCalendar(true)} style={{ cursor: 'pointer' }}>
                                    <Calendar size={18} className="icon-luxury v6-inner-icon" /><span className="v6-val-text">{departDate ? (tripType === 'roundtrip' ? `${departDate} - ${returnDate || '...'}` : departDate) : 'Izaberite datume'}</span>
                                    {showCalendar && (
                            ...
                            <div style={{ flex: 1 }}>
                                <div onClick={(e) => openClassDropdown(e, null)} className="v6-occupancy-trigger" style={{ position: 'relative', display: 'flex', alignItems: 'center', padding: '0 12px 0 44px' }}>
                                    <Plane className="icon-luxury" size={18} style={{ position: 'absolute', left: '14px' }} /><span style={{ flex: 1, marginLeft: '8px', fontWeight: 600, fontSize: '15px' }}>{selectedClassLabelSingle}</span><ChevronDown className="icon-luxury" size={14} style={{ opacity: 0.5, marginLeft: '4px' }} />
                                </div>
                            </div>
                            <button type="button" onClick={() => setShowAdvanced(!showAdvanced)} className={`v6-filter-btn ${showAdvanced ? 'active' : ''}`}><SlidersHorizontal className="icon-luxury" size={20} /></button>
                            <button onClick={handleSearch} className="btn-primary flight-hero-btn" style={{ background: 'linear-gradient(135deg, #7B2CBF, #9D4EDD)' }}><span style={{ whiteSpace: 'nowrap' }}>Pretraži letove</span><Send className="icon-luxury" size={18} style={{ stroke: 'white' }} /></button>
                        </div>
                    )}
                    ...
                    {tripType === 'multicity' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            {legs.map((leg, idx) => (
                                <div key={leg.id} className="v6-search-controls-row" style={{ alignItems: 'center', flexWrap: 'nowrap', gap: '8px', padding: '16px', background: 'var(--bg-surface)', borderRadius: '20px', border: '1.5px solid var(--border-color)', boxShadow: 'var(--shadow-sm)' }}>
                                    <div style={{ width: '32px', height: '32px', background: 'var(--brand-primary)', color: 'white', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '12px', flexShrink: 0 }}>{idx + 1}</div>
                                    ...
                                    <div style={{ flex: 1.2 }}><AirportAutocomplete value={leg.destination.code} onChange={(code, city) => updateLeg(leg.id, 'destination', { code, city })} placeholder="Odredište" icon={<Search className="icon-luxury" size={18} />} /></div>
                                    <div className="v6-ctrl-box" style={{ flex: 1 }}>
                                        <div className="v6-input-v6-styled" onClick={() => setActiveCalendarLeg(leg.id)} style={{ cursor: 'pointer', background: 'var(--bg-input)' }}>
                                            <Calendar size={18} className="icon-luxury v6-inner-icon" /><span className="v6-val-text">{leg.date || 'Datum'}</span>
                                            {activeCalendarLeg === leg.id && (
                                                <ExpediaCalendar startDate={leg.date} endDate={leg.date} initialFlexibleDays={0}
                                    ...
                                    <div style={{ flex: 0.8 }}>
                                        <div onClick={(e) => openClassDropdown(e, leg.id)} className="v6-occupancy-trigger" style={{ position: 'relative', display: 'flex', alignItems: 'center', padding: '0 12px 0 38px', background: 'var(--bg-input)' }}>
                                            <Plane className="icon-luxury" size={14} style={{ position: 'absolute', left: '12px' }} /><span style={{ flex: 1, marginLeft: '6px', fontWeight: 600, fontSize: '13px' }}>{CABIN_CLASSES.find(c => c.value === leg.cabinClass)?.label || 'Ekonomska'}</span><ChevronDown className="icon-luxury" size={12} style={{ opacity: 0.5 }} />
                                        </div>
                                    </div>
                                    <button onClick={() => removeLeg(leg.id)} disabled={legs.length <= 2} style={{ width: '40px', height: '52px', background: 'transparent', border: '2px solid #ef4444', borderRadius: '12px', color: '#ef4444', cursor: legs.length <= 2 ? 'not-allowed' : 'pointer', opacity: legs.length <= 2 ? 0.2 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Trash2 className="" size={18} /></button>
                                </div>
                            ))}
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' }}>
                                <button onClick={addLeg} className="btn-secondary" style={{ background: 'var(--bg-surface)', border: '1.5px dashed var(--border-color)', color: 'var(--text-muted)', padding: '12px 24px', borderRadius: '16px', height: '54px', cursor: 'pointer', display: 'flex', alignItems: 'center', fontWeight: 700 }}>
                                    <Plus className="icon-luxury" size={18} style={{ marginRight: '8px' }} /> Dodaj novi let
                                </button>
                                <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                                    <button type="button" onClick={() => setShowAdvanced(!showAdvanced)} className={`v6-filter-btn ${showAdvanced ? 'active' : ''}`}><SlidersHorizontal className="icon-luxury" size={20} /></button>
                                    <button onClick={handleSearch} className="btn-primary flight-hero-btn" style={{ minWidth: '240px' }}><span style={{ whiteSpace: 'nowrap' }}>Pretraži sve letove</span><Send className="icon-luxury" size={18} style={{ stroke: 'white' }} /></button>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {showClassDropdown && createPortal(
                        <div className="v6-portal-wrapper" onMouseDown={e => e.stopPropagation()}>
                            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 10000, background: 'transparent' }} onClick={() => setShowClassDropdown(false)}>
                                ...
                                >
                                    <div style={{ padding: '0 12px 12px 12px', fontSize: '11px', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid var(--border-color)', marginBottom: '8px' }}>Klasa leta</div>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                        {CABIN_CLASSES.map(cls => (
                                            <div 
                                                key={cls.value} 
                                                onClick={() => { if (activeClassLegId === null) setCabinClass(cls.value); else updateLeg(activeClassLegId, 'cabinClass', cls.value); setShowClassDropdown(false); }} 
                                                className="v6-class-option" 
                                                style={{ 
                                                    padding: '12px 14px', 
                                                    borderRadius: '12px', 
                                                    display: 'flex', 
                                                    ...
                                                    background: (activeClassLegId === null ? cabinClass === cls.value : legs.find(l=>l.id===activeClassLegId)?.cabinClass === cls.value) ? 'var(--brand-accent-light)' : 'transparent' 
                                                }}
                                            >
                                                <span style={{ fontWeight: 700, fontSize: '14px' }}>{cls.label}</span>
                                                {(activeClassLegId === null ? cabinClass === cls.value : legs.find(l=>l.id===activeClassLegId)?.cabinClass === cls.value) && <Check className="icon-luxury" size={16} strokeWidth={3} />}
                                            </div>
                                        ))}
                                    </div>
                                </div>
                    ...
                    {/* SHARED AIRLINE POPUP */}
                    {showAirlineDropdown && createPortal(
                        <div className="v6-portal-wrapper" onMouseDown={e => e.stopPropagation()}>
                            <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 10000, background: 'transparent' }} onClick={() => setShowAirlineDropdown(false)}>
                                <div 
                                    onClick={e => e.stopPropagation()} 
                                    className={`v6-prime-hub v6-luxury-popover ${isDark ? 'v6-dark' : ''}`}
                                    style={{ 
                                        position: 'absolute', 
                                        top: airlinePanelPos.top, 
                                        left: airlinePanelPos.left, 
                                        width: '320px', 
                                        padding: '12px'
                                    }}
                                >
                                    <div style={{ padding: '0 12px 12px 12px', fontSize: '11px', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid var(--border-color)', marginBottom: '8px' }}>Izaberi kompaniju</div>
                                    <div style={{ maxHeight: '300px', overflowY: 'auto', paddingRight: '4px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                        {AIRLINES.map(air => (
                                            <div 
                                                key={air.id} 
                                                onClick={() => { setAirline(air.id); setShowAirlineDropdown(false); }} 
                                                className="v6-class-option" 
                                                style={{ 
                                                    padding: '12px 14px', 
                                                    borderRadius: '12px', 
                                                    ...
                                                    color: 'var(--text-main)',
                                                    transition: 'all 0.2s ease',
                                                    background: airline === air.id ? 'var(--brand-accent-light)' : 'transparent' 
                                                }}
                                            >
                                                <span style={{ fontWeight: 700, fontSize: '14px' }}>{air.name}</span>
                                                {airline === air.id && <Check className="icon-luxury" size={16} strokeWidth={3} />}
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="v6-advanced-panel-flat-v2">
                            <div className="v6-filters-grid-v2">
                                ...
                                <div className="v6-filter-group-v2">
                                    ...
                                    <div onClick={openAirlineDropdown} className="v6-airline-select-v6" style={{ cursor: 'pointer' }}>
                                        <Building2 size={18} className="v6-icon-brand" />
                                        <span style={{ flex: 1, paddingLeft: '32px', fontWeight: 700, fontSize: '15px', color: 'var(--text-main)' }}>{selectedAirlineLabel}</span>
                                        <ChevronDown size={14} className="v6-icon-chevron" />
                                    </div>
                                </div>
                            </div>
            ...
            <style>{`
                ...
                .v6-trip-btn.active { background: var(--v6-accent); }
                .v6-trip-btn svg { stroke: var(--v6-text-primary) !important; stroke-width: 2.5; }
                .v6-trip-btn.active svg { stroke: var(--v6-accent-text) !important; }
                ...
                .v6-filter-group-v2 label { 
                    ...
                    color: var(--v6-text-secondary); margin-bottom: 12px; letter-spacing: 0.5px;
                }
                .v6-pill-group-v2 { display: flex; gap: 8px; flex-wrap: nowrap; overflow-x: auto; }
                .v6-pill-group-v2 button { 
                    padding: 10px 16px; border: 1.5px solid var(--v6-border); background: var(--v6-bg-card); 
                    border-radius: 10px; font-weight: 700; font-size: 13px; cursor: pointer;
                    display: flex; align-items: center; gap: 6px; color: var(--v6-text-primary);
                    transition: all 0.2s; white-space: nowrap;
                }
                .v6-pill-group-v2 button.active { background: var(--v6-accent); color: var(--v6-accent-text) !important; border-color: var(--v6-accent); }
                .v6-pill-group-v2 button.active svg { stroke: var(--v6-accent-text) !important; }

                .v6-airline-select-v6 {
                    height: 48px; border: 1.5px solid var(--v6-border); border-radius: 12px; background: var(--v6-bg-card);
                ...
                .v6-airline-select-v6 select {
                    ...
                    font-weight: 700; font-size: 14px; color: var(--v6-text-primary); appearance: none; cursor: pointer;
                }
                .v6-icon-brand { position: absolute; left: 14px; color: var(--brand-accent); }
                .v6-icon-chevron { position: absolute; right: 12px; opacity: 0.4; color: var(--text-main); }

                .v6-filters-switches-v2 { 
                    display: flex; align-items: center; gap: 24px; 
                    padding-top: 16px; border-top: 1.5px solid var(--v6-border); 
                }
                .v6-switch-label-v2 { display: flex; align-items: center; gap: 10px; font-weight: 700; font-size: 14px; color: var(--v6-text-primary); cursor: pointer; }
                .v6-switch-label-v2 input { width: 18px; height: 18px; accent-color: var(--v6-accent); cursor: pointer; }
                .v6-pill-tag { background: var(--v6-bg-section); padding: 8px 16px; border-radius: 100px; }

                /* REZULTATI / KLASE */
                .v6-class-option:hover { background: var(--v6-bg-section) !important; }
                .v6-input-v6-styled { 
                    height: 52px; 
                    border: 1.8px solid #1A234E !important; /* Debela Navy Ivica */
                    border-radius: 12px; 
                    background: #FFFFFF !important; /* Bela pozadina */
                    display: flex; 
                    align-items: center; 
                    padding: 0 12px; 
                    position: relative; 
                }
                .v6-inner-icon { margin-right: 10px; stroke: #1A234E !important; stroke-width: 2.5; flex-shrink: 0; }
                .v6-val-text { font-size: 14px; font-weight: 700; color: #1A234E; white-space: nowrap; overflow: hidden; }
                .v6-swap-circle { 
                    width: 44px; height: 44px; 
                    border: 1.8px solid #1A234E !important; 
                    border-radius: 50%; 
                    background: #FFFFFF; 
                    display: flex; align-items: center; justify-content: center; 
                    cursor: pointer; flex-shrink: 0; box-shadow: var(--v6-shadow-sm); z-index: 10; 
                    color: #1A234E; 
                }
                .v6-filter-btn { 
                ...
                .v6-filter-btn.active { background: var(--v6-accent); color: var(--v6-accent-text); border-color: var(--v6-accent); }
                .v6-filter-btn.active svg { stroke: var(--v6-accent-text) !important; }
                .flight-hero-btn { height: 52px; border_radius: 12px; padding: 0 24px; font-weight: 800; font-size: 14px; gap: 12px; }
                .v6-occupancy-trigger { 
                    height: 52px !important; 
                    border: 1.8px solid #1A234E !important; 
                    border-radius: 12px !important; 
                    background: #FFFFFF !important; 
                    transition: all 0.2s ease; cursor: pointer; color: #1A234E; 
                }
                .v6-occupancy-trigger:hover { background: var(--v6-bg-section) !important; }
            `}</style>
        </div>
    );
};
