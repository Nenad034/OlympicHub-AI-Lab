...
export interface DatabaseReservation {
    ...
    supplier_deadline?: string; // Date when payment must reach supplier
    client_deadline?: string;   // Date when client must pay agency
    cancellation_policy_json?: any; // Full policy for display
}
...
export async function saveDossierToDatabase(dossier: any): Promise<{ success: boolean; error?: string; data?: any }> {
    try {
        ...
        const dbRes: DatabaseReservation = {
            ...
            // Calculate deadlines
            supplier_deadline: firstItem?.supplierPaymentDeadline || firstItem?.cancellationPolicyRequestParams?.CancellationDate || undefined,
            client_deadline: (() => {
                const sDate = firstItem?.supplierPaymentDeadline || firstItem?.cancellationPolicyRequestParams?.CancellationDate;
                if (!sDate) return undefined;
                const d = new Date(sDate);
                d.setDate(d.getDate() - 2); // 2 days buffer for internal processing
                return d.toISOString().split('T')[0];
            })(),
            cancellation_policy_json: firstItem?.cancellationPolicy || firstItem?.cancellationPolicyRequestParams || undefined
        };
...
/**
 * Bulk get the count of reservations for multiple accommodations in the last 30 days.
 * This prevents making 100+ separate DB queries (which slows down the app significantly).
 */
export async function getBulkMonthlyReservationCounts(accommodationNames: string[]): Promise<Record<string, number>> {
    try {
        if (!accommodationNames || accommodationNames.length === 0) return {};
        ...
        accommodationNames.forEach(name => {
            ...
            const countForHotel = safeData.filter((r: any) =>
        ...
        console.error('[Reservation Service] Unexpected error in getBulkMonthlyReservationCounts:', error);
        return {};
    }
}
