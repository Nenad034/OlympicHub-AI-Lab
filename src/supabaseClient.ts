...
if (!supabaseUrl || !supabaseAnonKey || supabaseUrl.includes('your_') || supabaseAnonKey.includes('your_')) {
    console.warn("⚠️ ClickToTravel Hub: Supabase Cloud credentials missing. App running in OFFLINE/DEMO mode.");
    // Soft mock implementation to prevent crash while informing developer
    const mockError = (method: string) => ({
        data: null,
        error: { message: `🚀 ClickToTravel Hub: Supabase Cloud connection required for '${method}'. Create .env file with VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.` }
    });
